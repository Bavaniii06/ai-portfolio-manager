...
for stock in holdings:
    ...
    rebalance_data.append({
        'Stock': stock, 'Value': f"₹{portfolio_values[stock]:,.0f}",
        'Current %': f"{current_pct:.1f}%", 'Target %': f"{target}%", 'Action': action
    })
...
# === EXPORT ===
st.markdown("---")
st.download_button("📊 Export Analysis", df_rebalance.to_csv(index=False), 
                  "portfolio-rebalancing.csv", "text/csv")
st.success("✅ Dashboard LIVE! TCS overweight warning active!")
